...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// Expose React for the injected xyflow UMD (it externalizes react / react-dom).
...
@media (prefers-reduced-motion:reduce){.ws-card{animation:none}.ws-btn,.ws-node{transition:none}}
`;
		...
		function WorkflowNode({ data, selected }) {
			return jsxs("div", { className: "ws-node" + (selected ? " sel" : ""), children: [
				...
				data.files && data.files.length ? jsx("div", { className: "ws-fileRow", children: data.files.map((f) => jsx("span", { key: f.id, children: "● " + f.name })) }) : null
			] });
		}
		//#endregion
		...
		function WorkflowView({ t, sessionId }) {
			...
			react.useEffect(() => {
				...
				setNodes([{ id: "start-1", type: "workflow", position: { x: 40, y: 140 }, data: { title: t("node.start"), kind: "start", iconColor: "#FF9500" } }]);
			}, [ready]);
			...
			const addNode = (item) => {
				...
				const n = {
					id,
					type: "workflow",
					position: { x: 60 + Math.random() * 120, y: 60 + Math.random() * 140 },
					data: { title: item.label, kind: item.kind, sub: item.sub, iconColor: item.color, files: [] }
				};
			...
			const onConnect = (conn) => {
				...
				fetch("/api/dsh-workflow-studio/edge-intent", {
					...
					setOverlay({
						...
						onConfirm: (intent) => {
							const edge = {
								id: `e-${Date.now()}`, source: conn.source, target: conn.target,
								label: intent.label, type: "smoothstep",
								data: { kind: intent.kind, detail: intent.detail }
							};
			...
			return jsxs("div", { className: "ws-view", children: [
				...
				jsxs("div", { className: "ws-body", children: [
					...
					jsx("div", { className: "ws-canvas", children: jsx(RF.ReactFlow, {
						nodes, edges, nodeTypes, onConnect, fitView: true,
						children: [
							jsx(RF.Background, {}),
							jsx(RF.MiniMap, {}),
							jsx(RF.Controls, {})
