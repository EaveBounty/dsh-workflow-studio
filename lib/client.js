...
`;
		...
		function WorkflowNode({ data, selected }) {
			...
			return jsxs("div", { className: "ws-node" + (selected ? " sel" : "") + (bubbleMode === "float" ? " bubble-float" : " bubble-default"), children: [
				...
				data.kind === "action" ? jsx("select", {
					value: data.actionMode || "normal", className: "ws-modeSel",
					onClick: (e) => e.stopPropagation(),
					onChange: (e) => { if (actionModeHandler) actionModeHandler(data.id, e.target.value); },
					children: [
						jsx("option", { value: "normal", children: "普通" }),
						jsx("option", { value: "ptc", children: "PTC" }),
						jsx("option", { value: "loop", children: "Loop" })
					]
				}) : null,
		...
		function WorkflowView({ t, sessionId }) {
			...
			react.useEffect(() => { bubbleMode = bubbleModeState; pluginSettings.bubbleMode = bubbleModeState; }, [bubbleModeState]);
			react.useEffect(() => { rollbackHandler = rollbackNode; actionModeHandler = setActionMode; }, [nodes]);
			...
			const setActionMode = (id, mode) => {
				setNodes((prev) => prev.map((n) => n.id === id ? { ...n, data: { ...n.data, actionMode: mode } } : n));
			};
			...
			const exportWorkflow = () => {
				const data = { type: "dsh-workflow-studio", version: 1, name: "workflow", plan, status: "draft", nodes, edges };
				const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json" });
				const url = URL.createObjectURL(blob);
				const a = document.createElement("a");
				a.href = url; a.download = "workflow.dsh-workflow.json";
			...
			const importWorkflow = (file) => {
				...
				reader.onload = () => {
					try {
						...
						const n = Array.isArray(data.nodes) ? data.nodes.map((nd) => ({ ...nd, type: "workflow", data: { title: nd.data?.title || nd.id, kind: nd.data?.kind || "plan", files: [], ...nd.data } })) : [];
			...
			return jsxs("div", { className: "ws-view", children: [
				jsxs("div", { className: "ws-toolbar", children: [
					...
					jsx("div", { className: "ws-runbar", children: [
						...
						jsx("button", { className: "ws-btn", onClick: exportWorkflow, title: "导出为 JSON 分享", children: "导出" }),
						jsx("button", { className: "ws-btn", onClick: () => fileInputRef.current && fileInputRef.current.click(), children: "导入" }),
				...
				jsx("div", { className: "ws-foot", children: [
					...
					jsx("span", { className: "ws-status", style: { flex: "none" }, children: "← 在页面顶部标签可切回「对话」" })
				] }),
