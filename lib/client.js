...
@media (prefers-reduced-motion:reduce){.ws-card{animation:none}.ws-btn,.ws-node,.ws-fileRow,.ws-fileBtn,.ws-reviewItem{transition:none}.ws-edgeFlow{animation:none}}
`;
		...
		function WorkflowNode({ data, selected }) {
			const onAttach = data.onAttachFile;
		...
		function WorkflowView({ t, sessionId }) {
			...
			const addNode = (item) => {
				...
				const data = { title: item.label, kind: item.kind, sub: item.sub, iconColor: item.color, files: [], onAttachFile: attachFile };
